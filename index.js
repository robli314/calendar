function calendar(settings = {}) {
    ...
    const render = (date, locale) => {
        ...
        return `<calendar data-firstday="${config.info.firstDay}">
    ...
    const shiftWeekdays = (weekdays, firstDay) => {
        const shifted = [...weekdays];

        for(let i = 0; i < 8 - firstDay; i++) {
            shifted.splice(0,0, shifted.pop());
        }
        return shifted;
    }
    ...
    const weekdays = (firstDay, locale) => {
        ...
        return shiftWeekdays(weekDays, firstDay);
    }
    ...
    if (!config.info) {
        config.info = new Intl.Locale(config.locale).weekInfo || { firstDay: 7, weekend: [6, 7] };
    }

    return config.year ? [...Array(12).keys()].map(i => render(new Date(date.getFullYear(), i, date.getDate()), config.locale)).join('') : render(date, config.locale)
}
...
/* Init Demo */
app.innerHTML = calendar(app.dataset);
